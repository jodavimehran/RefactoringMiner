...
public class UMLOperationBodyMapper implements Comparable<UMLOperationBodyMapper> {
    ...
    public Set<Refactoring> getRefactorings() {
        ...
        for (VariableDeclaration leftSideVar : operation1.getAllVariableDeclarations()) {

            for (VariableDeclaration rightSideVar : operation2.getAllVariableDeclarations()) {
                if (leftSideVar.getVariableName().equals(rightSideVar.getVariableName())
                        && leftSideVar.getType().equals(rightSideVar.getType())
                        && !leftSideVar.getScope().equals(rightSideVar.getScope())
                ) {
                    refactorings.add(new ChangeVariableScopeRefactoring(leftSideVar, rightSideVar, operation1, operation2));
                }
            }
        }
        return refactorings;
